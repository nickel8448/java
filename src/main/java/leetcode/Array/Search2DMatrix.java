...
public class Search2DMatrix {
  ...
  public static boolean searchMatrix(
      int[][] matrix, int rowStart, int rowEnd, int colStart, int colEnd, int target) {
    ...
    if (midElement == target) {
      ...
      if (midRow != rowEnd || midCol != colStart)
        topRight = searchMatrix(matrix, rowStart, midRow, midCol, colEnd, target);
      if (rowStart == rowEnd && colStart + 1 == colEnd)
        if (matrix[rowStart][rowEnd] == target)
          if (matrix[midRow][midCol] < target) {
            if (midRow + 1 <= rowEnd) {
              findInRows = searchMatrix(matrix, midRow + 1, rowEnd, colStart, colEnd, target);
            }
          } else {
            if (midCol - 1 >= 0) {
              findInCols = searchMatrix(matrix, rowStart, rowEnd, colStart, midCol - 1, target);
            }
          }
